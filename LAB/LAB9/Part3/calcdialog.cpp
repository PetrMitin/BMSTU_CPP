...
QVBoxLayout *additionalKeysLayout = new QVBoxLayout();
...
if( _btnDescr[i].id >= 10 && _btnDescr[i].id < 33){
...
else if (_btnDescr[i].id >= POW && _btnDescr[i].id <= COS) {
    additionalKeysLayout->addWidget(button);
}
else
{ // кнопка "=" - помещаем в блок mainKeysLayout после gridLayout
    mainKeysLayout->addLayout(additionalKeysLayout);
...
// нашего класса
connect(m_pSignalMapper, SIGNAL(mappedInt(int)), this, SLOT(clicked(int))); //added
...
case SIN: {setNumEdit( qSin(getNumEdit()) ); break;};
case COS: {setNumEdit( qCos(getNumEdit()) ); break;};
